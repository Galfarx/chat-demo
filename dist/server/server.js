...
io.on('connection', function (socket) {
    ...
    socket.on('userChat', function (data) {
        var nick = (0, _util.sanitizeString)(data.nick);
        var message = (0, _util.sanitizeString)(data.message);
        var date = new Date();
        var time = ("0" + date.getHours()).slice(-2) + ("0" + date.getMinutes()).slice(-2);

        console.log('[CHAT] [' + time + '] ' + nick + ': ' + message);
        socket.broadcast.emit('serverSendUserChat', { nick: nick, message: message });
    });
});
